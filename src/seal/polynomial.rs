...
/// See [gf256](crate::seal::gf256) for more details.
...
impl Polynomial {
    ...
    /// Returns `Err(InvalidInput)` if any two x-values are identical (division by zero in GF256).
    pub fn interpolate(points: &[(GF256, GF256)]) -> FastCryptoResult<Self> {
        // Lagrangian interpolation, see e.g. https://en.wikipedia.org/wiki/Lagrange_polynomial
        let result = points
            .iter()
            .enumerate()
            .map(|(j, (x_j, y_j))| -> FastCryptoResult<Polynomial> {
                let basis = points
                    .iter()
                    .enumerate()
                    .filter(|(i, _)| *i != j)
                    .map(|(_, (x_i, _))| Self::monic_linear(-x_i) / &(x_j - x_i))
                    .collect::<FastCryptoResult<Vec<Polynomial>>>()?
                    .into_iter()
                    .product::<Polynomial>();
                Ok(basis * y_j)
            })

...
use self::Purpose::{Encryption, MAC};
use super::core::{Ciphertext, EncryptionInput, KEY_SIZE};
use super::utils::xor_unchecked;
...
fn encrypt_in_ctr_mode(key: &[u8; KEY_SIZE], msg: &[u8]) -> Vec<u8> {
    msg.chunks(KEY_SIZE)
        .enumerate()
        // xor_unchecked is correct here: the last chunk may be shorter than KEY_SIZE and
        // the PRF output is truncated to match — zip stops at the shorter slice by design.
...
#[cfg(test)]
mod tests {
    use super::{Aes256Gcm, Hmac256Ctr};
    use crate::seal::utils::generate_random_bytes;
    use crate::seal::core::KEY_SIZE;
    use rand::thread_rng;
